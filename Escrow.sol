...
contract Escrow {
	...
	function purchaseItem(address seller, string memory item) public payable {
		console.log("Request to purchase item '%s': buyer = %s", item, msg.sender);
	...
	function shipItem(uint txId) public {
		...
		console.log("Item shipped: '%s'", transactions[txId].item);
	...
	function itemReceived(uint txId) public {
		...
		console.log("Item received: '%s'", transactions[txId].item);
		...
		console.log("Payment released to seller %s", transaction.seller);
	}

	// Called by the buyer
	function cancelPurchase(uint txId) public {
		require(msg.sender == transactions[txId].buyer, "Not the buyer");
		console.log("Transaction canceled: '%s'", transactions[txId].item);
		completeTransaction(txId, Status.CANCELED);

		Transaction memory transaction = transactions[txId];
		(bool success, ) = transaction.buyer.call{ value: transaction.price }("");
		require(success, "Failed to send Ether");
		console.log("Payment refunded to buyer %s", transaction.buyer);
	}
	...
	function checkTransactionStatus(uint txId) public view returns (string memory) {
		Transaction memory transaction = transactions[txId];
		console.log("Item '%s': status = %s", transaction.item, statusToString(transaction.status));
		return statusToString(transaction.status);
	}

//	function checkMyBalance(address _address) public view returns (uint) {
